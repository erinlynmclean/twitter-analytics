...
```{r raw-and-clean-data, echo=FALSE,results=FALSE,message=FALSE}
...
raw_GA <- read_csv("~/Documents/Twitter Analytics/Raw Data/Web/2016-GA-Data.csv")
raw_GA$Date <-as.Date(raw_GA$Date) 
...
sum_categories<-raw_GA %>%  
  group_by(Date,Category) %>%
  summarize(page_sum = sum(Pageviews,na.rm=TRUE)) %>% 
  arrange(-page_sum)

top_five <- by(sum_categories, sum_categories["Date"], head, n=3)
top_five<-Reduce(rbind,top_five)
...
```{r category-display, echo=FALSE,results=FALSE,message=FALSE}
GAbar <- ggplot(data=top_five, aes(fill=Category, x=Date,y=page_sum))+
  geom_bar(position="dodge",stat="identity")+
  scale_fill_manual(values=c("#146660", "#1D244E", "#B3E1E7", "#79FD81","#19B369", "#000000"))
