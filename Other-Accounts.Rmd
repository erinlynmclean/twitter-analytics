...
influentialtwitter <- function(account_name, n){
    df <- get_timelines(account_name, n = n)
    df <- select(df, screen_name, created_at, text, display_text_width, reply_to_status_id,
                 reply_to_screen_name,is_retweet, favorite_count, retweet_count, hashtags,
                 media_type, mentions_screen_name,followers_count)
    return(df) #always goes at the end of the function
}
accounts<-c("@sciencegateways","@NASAEarthData","@arcticdatactr")

#accounts<-c("@4KarenStocks","@NASAEarthData","@sciencegateways","@yeshancqcq","@arcticdatactr","@CUAHSI" , "@kbene","@WeimingHu8","@opendap","@USGS_CDI", "@hsu000001", "@WHOI", "@EDIgotdata", "@obrienmobb", "@lstanish", "@GarretsonAlexis", "@twilamoon","@nsidc","@MGDSdata","@gmrt_bathy" ,"@drthevic" ,"@scdiggs","@MarshallXMa","@michael_ts0","@frankatsea","@NCAR_ACOM","@umbc","@sjskhalsa","@DataONEorg","@macromicropaleo","@tandfenviro","@RcaseyI","@smrariz","@NCAR_Science","@m_cragin","@ndlibraries","@NEON_sci","@nkmeyers","@ESSDIVE","@NCAR_CISL","@NCAR_RDA","@amckennafoster","@jensensunny","@DougSchuster3","@metamattj","@sjeanetteclark")
...
count_tweetssortedfav <- count_tweets %>% 
  #arrange(desc(normalfav)) %>% 
  group_by(screen_name)

count_tweetssortedrt <- byhour %>% arrange(desc(normalrt))
