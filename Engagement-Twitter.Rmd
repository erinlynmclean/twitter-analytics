...
```
```{r cleaning-data, echo=FALSE,results=FALSE,message=FALSE}
...
This graph illustrates that the Arctic Data Center's organic tweets are favorited often when we are mentioned and/or retweeted by some accounts (eg `r by_account_ADC_fav$account[1]` with a mean of `r round(by_account_ADC_fav$favorite_count[1],2)` favorites per tweet, `r by_account_ADC_fav$account[2]` with a mean of `r round(by_account_ADC_fav$favorite_count[2],2)` favorites per tweet, and `r by_account_ADC_fav$account[3]` with a mean of `r round(by_account_ADC_fav$favorite_count[3],2)` favorites per tweet). On the flip side, there are some accounts that when we’re mentioned, the tweets get no favorites at all (eg `r by_account_ADC_fav$account[16]` with a mean of `r round(by_account_ADC_fav$favorite_count[16],2)` favorites per tweet, `r by_account_ADC_fav$account[17]` with a mean of `r round(by_account_ADC_fav$favorite_count[17],2)` favorites per tweet, and `r by_account_ADC_fav$account[18]` with a mean of `r round(by_account_ADC_fav$favorite_count[18],2)` favorites per tweet).
...
Retweeting is a more direct measure of engagement, as this action puts the ArcticDataCtr name out to a wider variety of followers than just the `r twitter_summary_metrics$Number_Followers[1]` followers we have. This graph illustrates that the Arctic Data Center's organic tweets are retweeted often when we are mentioned and/or retweeted by some accounts (eg `r by_account_ADC_rt$account[1]` with a mean of `r round(by_account_ADC_rt$retweet_count[1],2)` retweets per tweet, `r by_account_ADC_rt$account[2]` with a mean of `r round(by_account_ADC_rt$retweet_count[2],2)` retweets per tweet, and `r by_account_ADC_rt$account[3]` with a mean of `r round(by_account_ADC_rt$retweet_count[3],2)` retweets per tweet). On the flip side, there are some accounts that when we’re mentioned, the tweets get fewer retweets (eg `r by_account_ADC_rt$account[15]` with a mean of `r round(by_account_ADC_rt$retweet_count[15],2)` retweets per tweet, `r by_account_ADC_rt$account[16]` with a mean of `r round(by_account_ADC_rt$retweet_count[16],2)` retweets per tweet, and `r by_account_ADC_rt$account[17]` with a mean of `r round(by_account_ADC_rt$retweet_count[17],2)` retweets per tweet).
...
```{r twitter-site-active, echo=FALSE,results=FALSE,warning=FALSE,message=FALSE,eval=FALSE}
...
The next step, using this data, will be to tweet more at different times so that there is more data on which to base an opinion.
...
tweetsusers<-ggplot(data=summarydata, aes(x=n, y=Twitter_Users))+
  ...
  labs(title='Site Users from Twitter vs Organic Tweets',
       ...
       y="Number of Users to arcticdata.io from Twitter")+
    stat_smooth(method="lm", se=FALSE, color="#B3E1E7",size=1)+
  stat_regline_equation(label.x = 20, label.y = 25)
...
siteusers<-ggplot(data=summarydata, aes(x=n, y=Site_View))+
  geom_point(color="#1D244E",size=2.5)+
  labs(title='Site Users vs Organic Tweets',
       x="Number of Organic Tweets per Month",
       y="Number of Users to arcticdata.io")+
    stat_smooth(method="lm", se=FALSE, color="#B3E1E7",size=1)+
  stat_regline_equation(label.x = 20, label.y = 2500)

equation2<-lm(Site_View~n, data=summarydata)
