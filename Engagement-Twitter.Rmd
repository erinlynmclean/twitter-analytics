...
#this doesn't work - reached out to Jeannette for help
influentialtwitter<-function(account_name){
  account_name<- get_timeline("@account_name", n= 3200)
  print(account_name)
}

influentialtwitter(US_APECS)

#Webscrape the content from Twitter account @PolarImpact
PolarImpact <- get_timeline("@PolarImpact", n= 3200)
#select only the columns I need
PolarImpact <- select(PolarImpact, status_id, created_at, text, display_text_width, reply_to_status_id, reply_to_screen_name,
                  is_retweet, favorite_count, retweet_count, hashtags, media_type, mentions_screen_name,followers_count)
#Take mentions out of the list
PolarImpact$mentions_screen_name <- lapply(PolarImpact$mentions_screen_name, paste0, collapse = ", ")
#grab only rows where this is true
mentions<-filter(PolarImpact, grepl("arcticdatactr", mentions_screen_name, fixed = TRUE))
#attach the account name to the end of the row so that later, when I rbind all of these together, I can make a bar graph with account as the x and number of retweets on the y
mentions$account<-paste0("PolarImpact")
