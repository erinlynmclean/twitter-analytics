...
```{r other-account-analysis, echo=FALSE,results=FALSE,warning=FALSE, message=FALSE,eval=FALSE}
...
influentialtwitter<-function(account_name, n){
  ...
  df$mentions_screen_name <- lapply(df$mentions_screen_name, paste0, collapse = ", ")
...
twitteraccounts<-c("@GlacierHub","@PolarImpact","@UArctic","@Polar_Research","@ArcticCouncil","@PridePolar","@PopePolar","@IARPCCollab","@Toolik" , "@IARC_Alaska","@ArcticResearch","@NSIDC","@US_APECS","@MOSAiCArctic")


# call things by vector index
result<-list() #creates empty list
for (i in 1:length(twitteraccounts)) {
  result[[i]] #makes it a list of dataframe
  <- influentialtwitter(twitteraccounts[i]) #for each i, it'll run the first, then the second
  
}

allresults<- do.call(rbind,result) #this only works with the same columns

toolik_results <- influentialtwitter("@Toolik",n=30)

```

One way to understand how impactful our Tweets are is by looking at the Twitter accounts that are mentioning @ArcticDataCtr in their Tweets. Influential accounts in the Arctic research space: GlacierHub (3700 followers), PolarImpact (r PolarImpact$followers_count[1] followers), UArctic (6900 followers), Polar_Research (8600 followers), ArcticCouncil (18000 followers), PridePolar (1800 followers), PopePolar (5500 followers), IARPCCollab (700 followers), Toolik (500 followers), IARC_Alaska (2000 followers), ArcticResearch (3500 followers), NSIDC (15500 followers), US_APECS (700 followers), and MOSAiCArctic (9000 followers).

Influential accounts in the data repository or open science space: DataONEorg (3600 followers), EDIgotdata (450 followers), NCEAS (9000 followers), USLTER (4600 followers), rOpenSci (29500 followers), NEON_sci (6100 followers), ESA_org (37000 followers), theAGU (65000 followers), ESIPfed (2200 followers), makedatacount (800 followers), and thecarpentries (7000 followers).

###Twitter as a gateway to arcticdata.io

With the power of Google Analytics, we can see what users of arcticdata.io are coming to the site from Twitter. This analysis shows that for every organic Tweet that we produce, we add `r coef(summary(equation))["n","Estimate"]` in unique visitors to our site. That may not seem like much averaged over the entire lifespan of the Twitter account, but if we only investigate times when the account was regularly updated (that is, more than 5 Tweets in a month)...
...
```{r twitter-site-active echo=FALSE,results=FALSE,warning=FALSE, message=FALSE}

#this time giving the active tweeting months more of a weight 
summarydataactive <-summarydata[summarydata$date>=2020-03-01, ] 

activetweetsusers<-ggplot(data=summarydataactive, aes(x=n, y=Twitter_Users))+
  geom_point(color="#1D244E",size=2.5)+
  labs(title='Site Users from Twitter vs Organic Tweets',
       x="Number of Organic Tweets per Month",
       y="Number of Users to arcticdata.io from Twitter")+
...
activetweetsusers<-activetweetsusers+theme_ADC()
activetweetsusers

```

### When is the best time of day to post tweets?
Only organic tweets are included in this analysis because engagements and impressions are not calculated the same way for retweets or replies.
