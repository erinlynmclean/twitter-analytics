...
rtweet::create_token(
  app = "arcticdataerin",
 consumer_key="ghq0BRPDALCzlJi6XeCfamHoz",
  consumer_secret="L6iMsi0E8s2HTOAxkIXDDJFsltWP8MHKHTvIQ3ty1lOXZKnYeC",
  access_token = "1227727460143259648-ZnA0WvysBE2cPnXmZtbBdwn6JYMmwS",
  access_secret = "Vo6HnIbN4uMDEPLNeBxazx9eInEjXmF7Qwz1NUeVHiiSL",
  set_renv = TRUE
)
...
setwd("~/Documents/Twitter Analytics/Raw Data/")
write.csv(monthlytweetsimp,file='imp_tweet_count.csv')
write.csv(monthlytweets,file='tweet_count.csv')
```
...
follower_count <- by_account_organic %>% 
...
One way to understand how impactful our Tweets are is by looking at the influential Twitter accounts that are mentioning @ArcticDataCtr (`r twitter_summary_metrics$Number_Followers[1]` followers) in their Tweets. Influential accounts in the Arctic research space that have mentioned our content organically, rather than just retweeting it: ArcticResearch (`r follower_count$followers_count[1]` followers), IARPCCollab (`r follower_count$followers_count[5]` followers), PopePolar (`r follower_count$followers_count[8]` followers), PridePolar (`r follower_count$followers_count[9]` followers), Toolik (`r follower_count$followers_count[11]` followers), and US_APECS (`r follower_count$followers_count[12]` followers).

Influential accounts in the data repository or open science space: DataONEorg (`r follower_count$followers_count[2]` followers), EDIgotdata (`r follower_count$followers_count[3]` followers), ESIPfed (`r follower_count$followers_count[4]` followers), NCEAS (`r follower_count$followers_count[6]` followers), NEON_sci (`r follower_count$followers_count[7]` followers), rOpenSci (`r follower_count$followers_count[10]` followers), and USLTER (`r follower_count$followers_count[13]` followers).

```{r other-account-analysis-graphs, results=FALSE,echo=FALSE,warning=FALSE, message=FALSE}
by_account_organic<- by_account_organic %>% summarise(
  total=n(),
...
by_account_organic_ADC<-rbind(by_account_organic,ADCmetrics)
by_account_organic

by_account_RT<-ggplot(data= by_account_organic, aes(x=account, y=retweet_count)) +
...
by_account_organic<- by_account_organic %>% arrange(desc(favorite_count))

by_account_fav<-ggplot(data= by_account_organic, aes(x=account, y=favorite_count)) +
...
by_account_ADC_fav<-by_account_organic %>% arrange(desc(favorite_count))
by_account_ADC_fav
```
